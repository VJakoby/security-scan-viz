  ...
  const normalizeNmapRows = useCallback((rows: VulnerabilityData[], file: File): ParsedData => {
    ...
    return {
      ...
      importInfo: {
        ...
        duplicateCount: Math.max(rows.length - normalizedVulnerabilities.length, 0),
      ...
      if (file.name.toLowerCase().endsWith('.csv')) {
        ...
        result = {
          ...
          importInfo: {
            ...
            rawRecordCount: normalized.vulnerabilities.length,
            duplicateCount: 0,
        ...
        if (normalized) {
          result = {
            ...
            importInfo: {
              ...
              fileName: file.name,
              recordCount: normalized.vulnerabilities.length,
              rawRecordCount: result.rows.length,
              duplicateCount: Math.max(result.rows.length - normalized.vulnerabilities.length, 0),
              reportName: normalized.reportName,
              autoMapped: true,
            },
          };
